document.querySelector('form').addEventListener('submit', (event) => {
    ...
    const mainApi = `https://newsapi.org/v2/top-headlines?country=${userInput}&apiKey=39c4aec4d86a443db3a5f47b4dfc5f7e`
    event.target.location.value = ''
    console.log(mainApi)
    fetch(mainApi).then(result =>{
        console.log('heyy', result)
        return result.json()
    }).then(news =>{
        console.log(news, 'please work')
    })
})
